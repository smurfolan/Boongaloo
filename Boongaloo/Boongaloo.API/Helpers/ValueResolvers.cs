...
namespace Boongaloo.API.Helpers
{
    ...
    public class InjectImageBaseForTripResolver : ValueResolver<Repository.Entities.Trip, string>
    {
        ...
        {
            string fullUri = Constants.BoongalooAPI + source.MainPictureUri;
            return fullUri;
        }
    }
    ...
    public class RemoveImageBaseForTripResolver : ValueResolver<DTO.Trip, string>
    {
        ...
        protected override string ResolveCore(DTO.Trip source)
        {
            ...
            // find
            var indexOfUri = partialUri.IndexOf(Constants.BoongalooAPI);
            if (indexOfUri > -1)
            {
                partialUri = partialUri.Substring(Constants.BoongalooAPI.Length);
            }
